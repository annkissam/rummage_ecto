defmodule Rummage.EctoTest do
  ...
  defp create_categories_and_products() do
    for x <- 1..4 do
      parent_category =
        %Category{category_name: "Parent Category #{10 - x}"}
        |> Repo.insert!()

      category =
        %Category{category_name: "Category #{x}", category: parent_category}
        |> Repo.insert!()
  ...
  test "rummage call with sort and assoc params returns the correct results" do
    ...
    assert Enum.all?(
             Repo.preload(products_1, :category),
             &(&1.category.category_name == "Category 1")
           )

    assert Enum.all?(
             Repo.preload(products_2, :category),
             &(&1.category.category_name == "Category 2")
           )

    assert Enum.all?(
             Repo.preload(products_3, :category),
             &(&1.category.category_name == "Category 3")
           )

    assert Enum.all?(
             Repo.preload(products_4, :category),
             &(&1.category.category_name == "Category 4")
           )
  ...
  test "rummage call with multiple associations in assocs search" do
    ...
    assert Enum.all?(
             Repo.preload(products, :category),
             &(Repo.preload(&1.category, :category).category.category_name =~ "Parent Category")
           )
    ...
    assert Enum.all?(
             Repo.preload(products, :category),
             &(Repo.preload(&1.category, :category).category.category_name == "Parent Category 9")
           )
  ...
  test "rummage call with multiple associations in assocs search and assoc sort" do
    ...
    assert Enum.all?(
             Repo.preload(products, :category),
             &(Repo.preload(&1.category, :category).category.category_name == "Parent Category 6")
           )
